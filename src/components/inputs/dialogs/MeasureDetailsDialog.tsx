...
export interface MeasureDetailsDialogProps extends DefaultDetailsDialogProps {
  // The REST API endpoint to upload files.
  apiEndpoint?: string;
}

const MeasureDetailsDialog: React.FC<MeasureDetailsDialogProps> = React.memo(
  (props) => {
    const { context } = props;
            ...
            <Grid item xs={6} md={4}>
              <Item>
                <InputControlFieldWrapper
                  id="project_types"
                  context={context}
                  options={getEnumAsObject(ProjectType, "label")}
